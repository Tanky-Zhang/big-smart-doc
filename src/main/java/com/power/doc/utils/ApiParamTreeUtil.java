...
public class ApiParamTreeUtil {
     ...
     * 构成层次结构
     */
    public static void rpcApiParamToTree(List<ApiParam> apiParamList) {
        if (!CollectionUtil.isEmpty(apiParamList)) {
            ApiParam param;
            StringBuilder append;
            for (Iterator<ApiParam> var1 = apiParamList.iterator(); var1.hasNext();
                    param.setField(append + param.getField())) {
                param = var1.next();
                int loop = param.getId();
